...
try:
    model_path = os.path.join(os.path.join(args.resultDir),'best_model.pth.tar')
    print("loading model from {}".format(model_path))
    model.load_state_dict(torch.load(model_path, map_location=device))
    print("model's been loaded")
except:
    checkpoint = torch.load(os.path.join(os.path.join(args.resultDir), 'best_model.pth.tar'), map_location=device)
